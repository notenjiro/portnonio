...
function shouldSkipFundSync(
  ...
  const isSupported = supported.includes(
    asset.symbol as (typeof supported)[number],
  );
...
async function saveFundNavPrice(asset: StockAsset) {
  const result = await fetchFundNav(
    asset.symbol as ReturnType<typeof listSupportedFundSymbols>[number],
  );
...
async function saveLiveStockPrice(asset: StockAsset) {
...
async function maybeBackfillCurrentMonthHistory(asset: StockAsset, dateKey: string) {
  if (asset.assetType === "fund") {
    return 0;
  }
...
export async function syncActiveFundNavs(
  options?: SyncOptions,
): Promise<SyncResult> {
...
export async function syncActiveStockPrices(
  options?: SyncOptions,
): Promise<SyncResult> {
  const assets = listStockAssets().filter((asset) => asset.assetType !== "fund");
...
export async function syncAllMarketData(
  ...
  return {
    ...
    items: [...funds.items, ...stocks.items].sort((a, b) =>
