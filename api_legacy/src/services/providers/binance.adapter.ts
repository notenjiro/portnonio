...
} from "../binance-futures.service.js";
import type {
...
function mapSpotHoldings(
  ...
  return balances
    ...
    .filter(
      (item): item is SpotHoldingView =>
        item !== null && Number.isFinite(item.value) && item.value > 0,
    )
    .sort((a, b) => b.value - a.value);
}

function mapFuturesPositions(
  positions: BinanceFuturesPosition[],
): FuturesPositionView[] {
  return positions
    .map((position): FuturesPositionView => {
...
export class BinanceProviderAdapter implements PortfolioProviderAdapter {
  readonly provider = "binance" as const;

  async getSummary(
    credentials: ProviderCredentials,
  ): Promise<ProviderSummary> {
    const spotInfo = await getBinanceAccountInfo(
      credentials.apiKey,
    ...
    const futuresAccount = await getBinanceFuturesAccountInfo(
      credentials.apiKey,
      credentials.apiSecret,
    );
  ...
  async getBreakdown(
    ...
    const [spotInfo, prices, futuresAccount, futuresPositionsRaw] =
      await Promise.all([
