...
function buildStock(seed, symbol, name, priceMin, priceMax, qtyMin, qtyMax) {
...
function buildFund(seed, symbol, name, navMin, navMax, unitsMin, unitsMax) {
...
export async function getInnovestXPortfolio(apiKey, apiSecret) {
...
export function summarizeInnovestXPortfolio(portfolio) {
    const stockValue = portfolio.stocks.reduce((sum, item) => sum + item.marketValue, 0);
    const fundValue = portfolio.funds.reduce((sum, item) => sum + item.marketValue, 0);
    const unrealizedPnL = portfolio.stocks.reduce((sum, item) => sum + item.unrealizedPnL, 0) +
