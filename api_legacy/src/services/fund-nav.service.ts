...
type SupportedFundSymbol = "SCBS&P500" | "K-USXNDQ-A(D)" | "SCBGOLDHRMF";

export type FundNavResult = {
  symbol: SupportedFundSymbol;
  nav: number;
  priceDate: string;
  source: string;
};
...
type FundSourceConfig = {
  symbol: SupportedFundSymbol;
  url: string;
  parser: (html: string) => ParsedFundPage;
};
...
function parseEnglishLongDate(value: string) {
...
function parseScbamFundPage(html: string): ParsedFundPage {
...
function parseKassetFundPage(html: string): ParsedFundPage {
  const clean = decodeHtml(normalizeWhitespace(html));
...
const FUND_SOURCES: FundSourceConfig[] = [
  {
    symbol: "SCBS&P500",
...
function getCacheKey(symbol: SupportedFundSymbol) {
  return `fund-nav:${symbol}`;
}

function markFundNavAsStale(value: FundNavResult): FundNavResult {
  return {
    ...value,
...
export function listSupportedFundSymbols(): SupportedFundSymbol[] {
  return FUND_SOURCES.map((item) => item.symbol);
}

export async function fetchFundNav(
  symbol: SupportedFundSymbol,
): Promise<FundNavResult> {
  const cacheKey = getCacheKey(symbol);
  const cached = getCache<FundNavResult>(cacheKey);
  ...
  try {
    ...
    const fallback = getCache<FundNavResult>(cacheKey);
...
export async function fetchAllSupportedFundNavs(): Promise<FundNavResult[]> {
  const results: FundNavResult[] = [];
